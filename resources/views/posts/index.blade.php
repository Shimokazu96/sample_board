                ...
                <div id="custom-search-input">
                    <div class="input-group col-md-12">
                        <form action="{{ route('posts.search') }}" method="get">
                            ...
                            <span class="input-group-btn" style="position: relative;top: -37px;right: -170px;">
        ...
        @foreach($posts as $post)
            <div class="card">
                ...
                <h5 class="card-title">カテゴリー：
                    <a href="{{ route('posts.index', ['$category_id' => $post->category_id]) }}"   $post->{{ $post->category->category_name }}</a>
                </h5>

                <h5 class="card-title">Tag：
                    @foreach($post->tags as $tag)
                    <a href="{{ route('posts.index', ['tag_name' => $tag->tag_name]) }}"   $post->#{{ $tag->tag_name }}</a>
                    @endforeach
                </h5>
